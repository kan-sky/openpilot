...
class Request:
  ...
  rx_offset: int = 0x8
  bus: int = 1
  # Carrot: Whether this query should be run on the first auxiliary panda (CAN FD cars for example)
...
class FwQueryConfig:
  ...
  def __post_init__(self):
    ...
    for request_obj in self.requests:
      ...
      # Carrot: No request on the OBD port (bus 1, multiplexed) should be run on an aux panda
    ...
    # Carrot: Add aux requests (second panda) for all requests that are marked as auxiliary
