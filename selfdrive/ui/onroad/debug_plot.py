...
class DebugPlot(Widget):
  ...
  def _render(self, rect: rl.Rectangle):
    ...
    sm = ui_state.sm
    # Kans
    data_ready = sm.alive.get("carState", False) and sm.alive.get("longitudinalPlan", False)
    ...
    ry = float(rect.y)
    # Kans: 플롯영역 정의(c3x용)
    left_margin, right_margin, bottom_margin = 400, 560, 580

    # 상단 텍스트 영역(상단마진 역할)
    title_h = 120

    # Kans: 플롯영역 계산
    W = int(rect.width - left_margin - right_margin)
    H = int(rect.height - bottom_margin)
    # Kans: 플롯표시영역
    plot_w = max(1, W)
    self.plot_x = rx + left_margin # 플롯 시작 x
    self.plot_y = ry + float(title_h) # 플롯시작 y

    self.plot_height = float(max(60, H - title_h))

    # plot_dx: PLOT_MAX 샘플을 표시영역에 꽉 채움
    self.plot_dx = float(max(1.0, plot_w / max(1, (PLOT_MAX - 1))))

    # Kans: 화면 깜박임 원인이라 주석처리.
    ...
    # rect 전체 투명 배경(필요없어 주석처리)
    #rl.draw_rectangle(x0, y0, x1 - x0, y1 - y0, rl.Color(0, 0, 0, 0))

    rl.draw_text_ex(self._font_display, title, rl.Vector2(x0 + 50, y0 + 8), 26, 0, rl.WHITE)
    min_max = f"min={self.plot_min:.2f}, max={self.plot_max:.2f}"
    rl.draw_text_ex(self._font_display, min_max, rl.Vector2(x0 + 50, y0 + 40), 26, 0, rl.WHITE)
