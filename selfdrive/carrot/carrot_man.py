...
class CarrotMan:
  ...
  def carrot_man_thread(self):
    ...
    #sm = messaging.SubMaster(['deviceState', 'carState', 'controlsState', 'longitudinalPlan'])
    ...
    while True:
      try:
        ...
        self.carrot_serv.update(data)
        #print(f"Received request: {message}")
        #response = {
        #    "status": "ok",
        #    "data": "Hello from Python ZeroMQ server!"
        #}
        socket.send(self.broadcast_message().encode('utf-8'))
        #continue
...
class CarrotServ:
  ...
  def _update_tbt(self):
    ...
    self.navTypeNext, self.navModifierNext, xTurnInfoNext = "invalid", "", -1
    if self.nTBTTurnTypeNext in turn_type_mapping:
      self.navTypeNext, self.navModifierNext, xTurnInfoNext = turn_type_mapping[self.nTBTTurnTypeNext]
