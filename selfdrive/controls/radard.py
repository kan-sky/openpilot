...
STICKY_FAR_DREL = 50.0
STICKY_MAX_DPATH_FAR = 1.8
...
class Track:
  ...
  def update(self, md, pt, ready, radar_reaction_factor, radar_lat_factor):
    ...
    self.jLead = pt.jLead
    self.yvLead = pt.yvRel if math.isfinite(pt.yvRel) else 0.0 # Kans: NaN/inf 방지. invalid 시 yvLead=0으로 처리하여 yRel_future가 NaN 되는 것 방지

    # Kans: radar yvRel이 NaN/inf/0에 가까운 경우가 많으므로
    # yRel 변화량으로 횡속도 추정후 yRel_future 계산에 사용
    if math.isfinite(self._prev_yRel):
      self.yRel_rate = (self.yRel - self._prev_yRel) / DT_MDL
    else:
      self.yRel_rate = 0.0
    self._prev_yRel = self.yRel

    if math.isfinite(pt.yvRel) and abs(pt.yvRel) > 0.01:
      self.yvLead = pt.yvRel
    else:
      self.yvLead = self.yRel_rate
    # yvLead가 invalid/0이면 yRel 변화량 기반 값을 future 계산에 사용
    self.yvLead = pt.yvRel if math.isfinite(pt.yvRel) and abs(pt.yvRel) > 0.01 else self.yRel_rate
  ...
  def sticky_dpath_limit(self) -> float:
    ...
    dist_extra = np.interp(self.dRel, [STICKY_FAR_DREL, 120.0], [0.0, 0.8])
    std_extra = STICKY_PATH_Y_STD_GAIN * self.sticky_path_y_std

    return float(np.clip(
      STICKY_MAX_DPATH + dist_extra + std_extra,
      STICKY_MAX_DPATH,
      STICKY_MAX_DPATH_FAR))
