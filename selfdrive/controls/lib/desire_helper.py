...
class DesireHelper:
  ...
  def update(self, carstate, modeldata, lateral_active, lane_change_prob, carrotMan, radarState):
    ...
    if not lateral_active or self.lane_change_timer > LANE_CHANGE_TIME_MAX:
      ...
      # ─ Lane change FSM ─
        ...
        if self.lane_change_state == LaneChangeState.off:
          ...
          # Kans: lane_change_prob가 너무 빨리 낮아질 수 있으므로
          # 최소 시작 유지시간(2s) 이후에만 finishing 허용
    ...
    return self.desire    self.left = SideState("left")
    self.right = SideState("right")
    ...
    if not lateral_active or self.lane_change_timer > LANE_CHANGE_TIME_MAX:
      ...
      # ─ Lane change FSM ─
        ...
        if self.lane_change_state == LaneChangeState.off:
          ...
          # Kans: lane_change_prob가 너무 빨리 낮아질 수 있으므로
          # 최소 시작 유지시간(2.5s) 이후에만 finishing 허용
          if self.lane_change_timer > 2.0 and lane_change_prob < 0.03 and self.lane_change_ll_prob < 0.01:
            self.lane_change_state = LaneChangeState.laneChangeFinishing
